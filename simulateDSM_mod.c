...
%[v,xn,xmax,y] = simulateDSM( u, ABCD, nlev[2], x0[0], si )
% or
%[v,xn,xmax,y] = simulateDSM( u, ntf, nlev[2], x0[0], si )
%
...
%
%s_i scales the output of the quantizer that is used for the feedback. 
%The output value itself is not scaled.
%
...
double 
    ...
    *si,    /* Points into the modulation array. */
    *x,		/* The current state. */
...
void checkArgs(int nlhs, mxArray **plhs, int nrhs, const mxArray **prhs) {
	...
	*pABCD = 0;		/* D2 */
    ...
    psi = mxCreateDoubleMatrix(nq, N, mxREAL);
    ...
    if(nrhs>=5)
        ...
        if(nsi==nq&&msi==N)
            ...
            fatalError("For each quantizer, a modulating signal must be defined.");
        }
        else {
        si = mxGetPr(psi);
        for( iN=0; iN<N; iN++){
...
{
    int i,j,t, qi;
    double *pABCD, *ptr, *ptrsi, *pxn, tmp;
	...
	for( i=0, pxn=xn; i<order; ++i ){
		...
		tmp += *pABCD * *ptr++;
	    for( ptr=v, j=0, ptrsi=si; j<nq; ++j, pABCD += ABCD_rows )
		tmp += *pABCD * *ptr++ * *ptrsi++;
	    *pxn++ = tmp;
	    }
	u += nu;
